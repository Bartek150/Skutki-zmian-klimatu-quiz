...
async def quiz(ctx):
    ...
    if pytanie == -1:
        ...
        await ctx.send(f"      Masz {int(time/60)} minutę aby to przeczytać i zapoznać się z tematem!      ",delete_after=time)
    ...
    if pytanie - max_points <= 0 and pytanie >= 0: #Jeśli wartość różnicy zmiennej pytanie i max_points jest mniejsza niż 2,kod zadziała normalnie(W wyniku wyświetlenia zagadnienia wartość zmiennej pytanie rośnie o 1)
        if pytanie == 0:
...
async def check(ctx,user_answer):
    ...
    if max_points < pytanie or (max_points == 5 and pytanie == 6):
        if pytanie == 1:
            if user_answer == "B":
                points += 1
                max_points += 1
            ...
            #jeśli pytanie = 5 i max_points = 5
            pytanie = -1
            points = 0
            max_points = 0
            score = 0
...
#    else:
 #       ctx.send("Nie możesz tego pominąć!")
#    await ctx.send(pytanie)
